/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define MAG_INT_Pin GPIO_PIN_3
#define MAG_INT_GPIO_Port GPIOE
#define BARO_INT_Pin GPIO_PIN_4
#define BARO_INT_GPIO_Port GPIOE
#define ACCEL_INT_Pin GPIO_PIN_5
#define ACCEL_INT_GPIO_Port GPIOE
#define GYRO_INT_Pin GPIO_PIN_6
#define GYRO_INT_GPIO_Port GPIOE
#define SPI1_IMU_ACCEL_CS_Pin GPIO_PIN_4
#define SPI1_IMU_ACCEL_CS_GPIO_Port GPIOA
#define SPI1_IMU_GYRO_CS_Pin GPIO_PIN_4
#define SPI1_IMU_GYRO_CS_GPIO_Port GPIOC
#define SPI2_FLASH_CS_Pin GPIO_PIN_14
#define SPI2_FLASH_CS_GPIO_Port GPIOB
#define GNSS_NRST_Pin GPIO_PIN_10
#define GNSS_NRST_GPIO_Port GPIOD
#define GNSS_LNA_EN_Pin GPIO_PIN_11
#define GNSS_LNA_EN_GPIO_Port GPIOD
#define GNSS_PSS_Pin GPIO_PIN_12
#define GNSS_PSS_GPIO_Port GPIOD
#define LED_H_Pin GPIO_PIN_0
#define LED_H_GPIO_Port GPIOD
#define LED_G_Pin GPIO_PIN_1
#define LED_G_GPIO_Port GPIOD
#define LED_F_Pin GPIO_PIN_2
#define LED_F_GPIO_Port GPIOD
#define LED_E_Pin GPIO_PIN_3
#define LED_E_GPIO_Port GPIOD
#define LED_D_Pin GPIO_PIN_4
#define LED_D_GPIO_Port GPIOD
#define LED_C_Pin GPIO_PIN_5
#define LED_C_GPIO_Port GPIOD
#define LED_B_Pin GPIO_PIN_6
#define LED_B_GPIO_Port GPIOD
#define LED_A_Pin GPIO_PIN_7
#define LED_A_GPIO_Port GPIOD

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
